...
pub struct SignStage1Input {
    ...
    pub sig_secret_private: PartyPrivate,
}
#[derive(Clone, Debug, Serialize, Deserialize)]
...
pub fn sign_stage2(input: &SignStage2Input) -> Result<SignStage2Result, ErrorType> {
    ...
    for j in 0..input.l_ttag - 1 {
        ...
        // beta_gamma is a secret value and needs to be encrypted with a key only know to party ind.
...
pub fn sign_stage7(input: &SignStage7Input) -> Result<SignStage7Result, ErrorType> {
    let res_sig = input.local_sig_vec[0].output_signature(input.local_sig_vec.clone(), input.a_ij_vec.clone());
...
mod tests {
    ...
    fn test_sign_orchestration_all() {
        ...
        .unwrap();
        let sig_secrets = keygen_orchestrator(Parameters {
            share_count: 3,
            threshold: 1,
        })
        .unwrap();
        ...
        let sign_result = orchestrate_sign(&s[..], &msg, &keypairs, &sig_secrets);
    ...
    pub fn orchestrate_sign(
        ...
        sigsecret_result: &KeyPairResult,
    ) -> Result<(), ErrorType> {
        ...
        (0..ttag).map(|i| i).for_each(|i| {
            let input = SignStage1Input {
                ...
                sig_secret_private: PartyPrivate::set_private(
                    sigsecret_result.party_keys_vec[s[i]].clone(), 
                    sigsecret_result.shared_keys_vec[s[i]].clone()
                ),
            };
            write_input!(
                i as u16,
        ...
        let _message_bn_l = HSha256::create_hash(&[&BigInt::from_bytes(bytes_to_sign)]);
