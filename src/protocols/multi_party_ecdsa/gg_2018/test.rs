...
mod tests {
    ...
    pub fn keygen_t_n_parties(
        ...
        //test
        let xi_vec = (0..t as usize + 1)
            .map(|i| shared_keys_vec[i].x_i.clone())
            .collect::<Vec<FE>>();
        let x = vss_scheme_for_test[0]
            .clone()
            .reconstruct(&index_vec[0..t as usize + 1], &xi_vec);
    ...
    fn sign(t: u16, n: u16, ttag: u16, s: Vec<usize>) {
        ...
        let sign_keys_vec = (0..ttag as usize)
        ...
        let _g_gamma_i_vec = (0..ttag as usize)
            .map(|i| sign_keys_vec[i].g_gamma_i.clone())
            .collect::<Vec<GE>>();

        let R_vec = (0..ttag as usize)
            .map(|_| {
                // each party i tests all B = g^b = g ^ gamma_i she received.
                let b_proof_vec = (0..ttag as usize)
                    .map(|j| {
