...
mod tests {
    ...
    fn test_full_key_gen() {
        let (party_one_first_message, comm_witness, ec_key_pair_party2) =
            party_one::KeyGenFirstMsg::create_commitments_with_fixed_secret_share(ECScalar::from(
                &BigInt::sample(253),
            ));
        let (party_two_first_message, _ec_key_pair_party1) =
        ...
        let paillier_key_pair =
            party_one::PaillierKeyPair::generate_keypair_and_encrypted_share(&ec_key_pair_party2);
